...
// Initialize Firebase Admin - moved to lib/firebase.ts to avoid duplicate initialization
// admin.initializeApp() is called in lib/firebase.ts with a check for existing apps
...
async function _addTask(userId: string, crmType: CRMType, params: any): Promise<void> {
  ...
  switch (crmType) {
    case "wise_agent":
      // @ts-ignore - task params mismatch
...
async function _getContacts(userId: string, crmType: CRMType, params: any): Promise<any[]> {
  ...
  switch (crmType) {
    case "wise_agent":
      ...
      return wiseResult.contacts || wiseResult || [];
    case "follow_up_boss":
